...
本次检查领取入口及 rCore、Rust 进阶、ArceOS 三门课的全部自有自动化 Python 代码、回归测试、Actions 工作流、Issue 表单、课程配置和操作文档。核对已有学员仓库与模板的公共文件、rCore 的全部章节分支及部署差异。上游第三方依赖不属于此次自有代码审查。另按维护者要求，在 Alayfolk64 的学员仓库实际完成部分实验，验证正分上传；课程模板保留原题，评分规则、测试和 OpenCamp 后台未修改。
...
以下运行均由向学员仓库 push 实际解答自动触发。三门课的上传作业成功，成绩文件记录 `upload.status=accepted`；随后只读查询 OpenCamp 秋冬季训练营（campId 119）的公开排行榜，确认 Alayfolk64 的分数分别为 11、100、100。

| 课程 | 实际完成 | 正式评测 | OpenCamp 分数 |
| --- | --- | --- | --- |
| 2074 Rust 进阶 & OS 入门 | 线程、互斥锁、通道三个练习包；本地 19 个单元测试及 4 个文档测试通过 | [运行 34781684490](https://github.com/2026f-autotest/2026f-oscamp-base-Alayfolk64/actions/runs/34781684490)，24 个练习包中 3 个通过 | 11/100 |
| 2073 rCore | ch3 的逐任务系统调用计数、trace 字节读写及实验报告 | [运行 34781704390](https://github.com/2026f-autotest/2026f-rcore-Alayfolk64/actions/runs/34781704390)，官方检查 7/7，报告检查通过 | 100/500 |
| 2078 组件化操作系统 | 带 ANSI 颜色与复位的彩色输出 | [运行 34781685108](https://github.com/2026f-autotest/2026f-arceos-Alayfolk64/actions/runs/34781685108)，QEMU 实际执行，6 个练习中 1 个通过 | 100/600 |

Rust 与 ArceOS 还有未完成题目，`Mark unfinished exercises` 按既有规则使测试作业显示失败；真实部分分数照常保存并上传。rCore 本章测试及上传全部成功。

### 领取、重试及部署

- 三个真实申请 [#4](https://github.com/2026f-autotest/enroll/issues/4)、[#5](https://github.com/2026f-autotest/enroll/issues/5)、[#6](https://github.com/2026f-autotest/enroll/issues/6) 均读取申请人 Alayfolk64，复用其已有学员仓库，等待对应配置检查成功后回复并关闭。配置运行分别为 34780404871、34780423053、34780476617。
- 在先前的基线评测中，仅重跑 Rust 上传作业（运行 34780362121 的 attempt 2），成功复用原附件 ID 10324456856；ArceOS 同样以运行 34780364181 的 attempt 2 复用原附件 ID 10325016097。没有因 attempt 变化丢失附件；两次上传均得到 accepted。上表为随后完成题目后的当前正分结果。
- 领取入口 35 项回归测试通过；三门课的云端自动化回归运行 34780355860、34780357930、34780360015 均成功。HTTP 故障和长时间限流仍以故障注入验证，没有向 GitHub 人为制造限流。
- 最终全量核验 6 个课程及学员仓库的 22 个课程分支：本地与远端 SHA 相同，所有分支中的公共建仓模块与领取入口逐字一致；模板仅变更审查清单内的自动化和文档，学员仓库额外变更上述解答与报告。
- 先前的真实临时建仓验证：配置失败运行 34779152195 后正式仓库名不存在；配置成功运行 34779220005 后才发布正式名称，改名前后 repository ID 都是 1368819382；准备期间的运行 34779166658 跳过评测和上传。专用验证仓库已归档。
